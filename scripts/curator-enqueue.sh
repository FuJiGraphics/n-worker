...
#   요청 JSON 필수 키: mode(register|record|targeted|sweep|model-refresh|harness-refresh), project_root, slug, stack,
...
CALLER="$("$PY" -c "import json,sys,re; print(re.sub(r'[^A-Za-z0-9_-]','_',json.load(open(sys.argv[1]))['caller'])[:40])" "$REQ")"
ID="${TS}-${CALLER}-$$"
"$PY" - "$REQ" "$C/queue/$ID.json" "$ID" <<'PY'
...
if [ "$WAIT" -gt 0 ]; then
  for i in $(seq 1 "$WAIT"); do
    if [ -f "$C/done/$ID.json" ]; then "$PY" -c "import json,sys; d=json.load(open(sys.argv[1])); print(d.get('status','?')); print(d.get('summary',''))" "$C/done/$ID.json"; exit 0; fi
    sleep 1
  done
