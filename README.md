...
<table align="center">
  <tr>
    ...
    <td width="50%" align="center">
      <img src="docs/img/demo-review.svg" alt="P4 위험 기반 리뷰 - 컴파일과 테스트 뒤 실제 diff 를 리뷰어가 확인하고 지적을 반영하는 화면" width="410"><br>
      <sub><b>실제 변경을 위험만큼 검토한다.</b><br>컴파일과 테스트가 못 보는 것만 리뷰어가 실제 diff 에서 확인한다.</sub>
    </td>
  </tr>
  <tr>
    <td width="50%" align="center">
...
인터뷰로 의도를 확정하고, 플랜을 승인받은 뒤 기존 구현을 본떠 구현하고, 컴파일, 테스트와 위험 기반 리뷰로 검증하고,
...
| 구조 위반, 중복 구현, 문자열 키 오타는 컴파일이 잡지 못한다 | **위험 기반 리뷰** - 자동 검증이 놓치는 것만 리뷰어가 실제 diff 에서 확인한다. 저위험 0명, 일반 1명, 고위험 2명 |
...
- **리뷰는 실제 변경에, 위험만큼만.** 동작의 근거는 컴파일과 테스트다. 리뷰어는 자동 검증이 놓치는 것(기존 구조 이탈, 중복, 문자열 키와 리소스 참조, 실패 비용이 큰 경로)만 실제 diff 에서 확인하고, 최초 투입은 최대 2명이다.
